...
def compute_frip_score(
    bw_file: str,
    peak_regions: List[Tuple[str, int, int]],
    chromosome_sizes: Dict[str, int]
) -> Dict[str, float]:
    """
    Calculate Fraction of Reads in Peaks (FRiP) score for QC.
    ...
    --------
    dict
        FRiP score and related metrics
    """
    with pyBigWig.open(bw_file) as bw:
        # Total signal in peaks
        peak_signal = sum(
            (bw.stats(c, s, e, type="sum")[0] or 0)
            for c, s, e in peak_regions
        )
        
        # Total signal genome-wide (sample chromosomes)
        total_signal = 0
        for chrom, size in list(chromosome_sizes.items())[:5]:  # Sample first 5 chroms
            try:
                sig = bw.stats(chrom, 0, size, type="sum")[0] or 0
                total_signal += sig
            except:
                continue
        
        frip = peak_signal / (total_signal + 1e-10)
